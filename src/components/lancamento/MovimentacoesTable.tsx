...
export default function MovimentacoesTable({ rows, onAdd, onUpdate, onDelete }: Props) {
  return (
    <div>
      ...
      <div className="text-[11px] text-muted italic px-3 py-1.5 bg-[#FFF8E1] border-l-[3px] border-amarelo">
      ...
      <div className="bg-card border border-border border-t-0 rounded-b-lg overflow-hidden">
        <div className="grid grid-cols-[2fr_1.2fr_1fr_32px] bg-[#F4F5FA] border-b border-border px-3 py-1.5">
        ...
        {rows.map(row => (
          <div
            key={row.id}
            className="grid grid-cols-[2fr_1.2fr_1fr_32px] px-3 py-2 items-center gap-2 border-b border-border last:border-0 odd:bg-white even:bg-[#FAFBFD] hover:bg-[#F0F4FF]"
          >
            <input
              type="text"
